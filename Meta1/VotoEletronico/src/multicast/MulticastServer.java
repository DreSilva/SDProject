...
public class MulticastServer extends Thread {
     ...
     * Lança a thread multicast.Console(lê CC's da consola do server para fazer a identificação e login do eleitor)
     * @param args
     * @throws IOException
     */
    public static void main(String[] args) throws IOException {
        ...
        if (args.length == 1) {
            ...
            System.out.println("Corra com o número de departamento: java multicast.MulticastServer <departamento>");
            System.exit(0);
        }
...
class Login extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            do {
                ...
                try {
                    ...
                    if (voto.login(arrOfStr[0], arrOfStr[1], this.CC)) {
                        ...
                        message = "server|" + clientID + ";cmd|logged off;msg|Wrong multicast.Login.Try again: ";
                        buffer = message.getBytes();
                        group = InetAddress.getByName(MULTICAST_ADDRESS);
                        packet = new DatagramPacket(buffer, buffer.length, group, PORT);
                        socket.send(packet);
            ...
            } while (message.equals("server|" + clientID + ";cmd|logged off;msg|Wrong multicast.Login.Try again: "));
        } catch (IOException | NotBoundException e) {
            e.printStackTrace();
        } finally {
            socket.close();
